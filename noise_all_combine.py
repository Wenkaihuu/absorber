...
data_path = '/home/p/pen/wkhu/scratch/map_result_gbt_absorb/maps/9maps_1hr/'
...
for part in parts:
    ...
    #freq_correct = [1.00007374848,1.00003189652,0.999988093128,0.999949057896,0.999928136691]
    ...
    sr_resultT = np.zeros(np.shape(noise_inv_combT[0]))
    
    print np.shape(noise_inv_combT)
    #assign the data or noise from different sky pixels to the frequency which have been shifted according to the doppler correction,use the ∑data_k*noise_inv_i/∑noise_inv_i to weight the data,use the ∑noise_inv_i/(∑noise_inv_i)**2 to weight the noise

    for i in np.arange(len(noise_inv_combT[0])):
        for j in np.arange(len(noise_inv_combT[0][0])):
            noise_combT = []
            mean = noise_inv_combT[0][i][j]+noise_inv_combT[1][i][j]+noise_inv_combT[2][i][j]+noise_inv_combT[3][i][j]+noise_inv_combT[4][i][j]
            for k in np.arange(5):
                #wdata_combT.append(data_combT[k][i][j]*noise_inv_combT[k][i][j]/noise_inv_combT[k][i][j].sum())
    ...
    np.save("/home/p/pen/wkhu/scratch/map_result_gbt_absorb/maps/9maps_1hr/sr_noise_"+part+"_9parts_1hr"+sec+".npy",sr_result)
